...
for j in range(nshifts):
    scores[:,0,j] = [np.sum([matrix[Nlookup[c], i+j] for i,c in enumerate(seq)]) for seq in seqs]
    scores[:,1,j] = [np.sum([matrix[3-Nlookup[c], i+j] for i,c in enumerate(seq[::-1])]) for seq in seqs]
...
out.to_csv(sys.argv[1][:-4] + "_SelexGLM.tsv", sep="\t", index=False)
